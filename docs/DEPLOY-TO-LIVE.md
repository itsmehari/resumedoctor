...
| `ZEPTOMAIL_SEND_TOKEN` | ✅ | ZeptoMail Send Mail token; also `EMAIL_FROM` on verified domain |
...
| Check fails | Fix |
|-------------|-----|
| ZEPTOMAIL_SEND_TOKEN | Vercel → Env Vars. Token from ZeptoMail → Agent → SMTP/API → Send Mail Token. Set `EMAIL_FROM` to a verified sender on that domain. |
